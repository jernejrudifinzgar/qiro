...
class QtensorQAOAExpectationValuesQUBO(ExpectationValues):
    ...
    def __init__(self, problem, p, pbar=True, gamma=None, beta=None, initialization='random', variation='QIRO', regularity=3, opt=torch.optim.RMSprop, opt_kwargs=dict(lr=0.001), backend=qtensor.contraction_backends.TorchBackend(), ordering_algo='greedy'):
        ...
        # for fixed angles initialization with optimization
        if initialization=='fixed_angles_optimization':
            with open('./Experiments/MAXCUT_RQAOA/scripts/angles_regular_graphs.json', 'r') as file:
                data = json.load(file)
    ...
    #@lru_cache
    def energy_peo(self):
        """Calculating contraction order and saving it for later contraction"""
        opt = qtensor.toolbox.get_ordering_algo(self.ordering_algo)
        peos = {}
    ...
    def energy_loss(self):
        """Calculates single correlations and sums them up to get the loss function. Also saves correlations in list."""
        sim = qtensor.QtreeSimulator(backend=self.backend)
        composer = qtensor.TorchQAOAComposer_QUBO(self.problem.graph, self.problem.matrix, self.problem.position_translater, gamma=self.gamma, beta=self.beta)
        self.loss = torch.tensor([0.])
        self.E_nodes = {}
    ...
    def return_loss_func(self, *args):
        """Returns loss function explicitly. This is required for some functions"""
        l=self.p
        gamma = []
        beta = []
        ...
        for i in range(self.p):
            gamma.append(args[i])
            beta.append(args[i+self.p])

        gamma = torch.stack(gamma)
        beta = torch.stack(beta)

        sim = qtensor.QtreeSimulator(backend=self.backend)
        composer = qtensor.TorchQAOAComposer_QUBO(self.problem.graph, self.problem.matrix, self.problem.position_translater, gamma=gamma, beta=beta)
        self.loss = torch.tensor([0.])
        self.peos = self.energy_peo()
    ...
    def create_expect_val_dict(self):
        """Builds dictionary with all correlations. This is used in the shrinking step."""
        self.expect_val_dict={}
        max_expect_val = 0
        max_expect_val_list = []
    ...
    def calc_expect_val(self):
        """Calls function to create dictionary with correlations and returns highest correlations"""
        self.peos = self.energy_peo()
        self.energy_loss()
    ...
    def calc_hessian(self):
        """Calculates hessian of loss function."""
        input = []
        for i in self.gamma:
            input.append(i)
        for i in self.beta:
            input.append(i)
        input = tuple(input)
        hess = hessian(self.return_loss_func, input)
    ...
    def optimize(self, steps=50, **kwargs):
        """Basic optimization function. Depending on which initialization method is chosen, different further optimizations are carried out."""
        ...
        elif self.initialization == 'interpolation':
    ...
    def optimize_general(self, steps, **kwargs):
            """Actual pytorch optimization is carried out here."""
            random.seed()
            self.peos = self.energy_peo()
    ...
    def optimize_transition_states(self, steps, **kwargs):
        """Optimization routine with using transition states."""
    ...
    def optimize_interp(self, steps, **kwargs):
        """Optimization with interpolation initialization."""
        if self.p==1:
            expectation_value_single = SingleLayerQAOAExpectationValues(self.problem)
