...
def plot_energy__different_opt_init_n_30(ns, ps, regularity, graphs):
    ...
    for n in ns: 
        ...
        for p in ps: 
            ...
            for graph, i in zip(graphs, range(len(graphs))):
                ...
                #with open(my_path + f'/data/nodes_{n}_reg_{regularity}_graph_{graph}_initialization_fixed_angles_optimization_opt_SGD_lr_{0.0001}_version_1.pkl', 'rb') as file:
                    #data = pickle.load(file)
                #energy_single = data['analytic_single_p']['energy']
                #plt.scatter([1], energy_single, color=colors[0], label=f'Grid search with analytic expression')
                ...
                for argument in arguments:
                    ...
                    plt.plot(list(range(len(losses))), losses, color=colors[counter], label=f'{argument[2]} with {argument[1]} and lr={argument[0]} optimization')
                    counter += 1
