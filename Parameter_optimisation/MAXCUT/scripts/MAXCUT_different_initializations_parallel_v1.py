...
def MAXCUT_QAOA_optimization_individual_initializations(graphs, ns, regularity, max_p=3, parallel=True):
    """Calculates and saves QAOA optimization energies of given graphs, parameter initializations and ps. Different optimizers, learning rates and initialization methods are specified in the following."""
    ...
    if parallel==True:
        ...
        pool.starmap(individual_MAXCUT_QAOA_optimization_single_initialization, arguments_list)
...
def MAXCUT_QAOA_optimization_all_initializations(graphs, ns, regularity, max_p=3, parallel=True):
    """Calculates and saves QAOA optimization energies of given graphs and ps. Different optimizers and learning rates are specified in the following. Here, this is done for all different initializations of random, fixed angles, interpolation and transition states"""
    optimizers_list = ['SGD', 'RMSprop', 'Adam']
    learning_rates_SGD = [0.0001, 0.0005, 0.001]
    learning_rates_RMSprop = [0.001, 0.005, 0.01, 0.05]
    learning_rates_Adam = [0.001, 0.005, 0.01, 0.05]
    ...
    if parallel==True:
        ...
        num_processes=len(arguments_list)
        pool = mp.Pool(num_processes) 
        pool.starmap(individual_MAXCUT_QAOA_optimization_all_initializations, arguments_list)
...
if __name__ == '__main__':
    # set which graphs should be analysed:
