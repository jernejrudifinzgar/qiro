    ...
    for p in ps: 
        ...
        if p==1:
            ...
            for j in range(p):
                ...
                gamma_ts.insert(j, 0)
                beta_ts.insert(j, 0)
                expectation_values_qtensor_transition = QtensorQAOAExpectationValuesMAXCUT(problem, p, gamma=gamma_ts, beta=beta_ts, pbar=False)
...
def fixed_angles(problem, regularity, ps):
    ...
    for p in ps:
        ...
        expectation_values_qtensor_fixed = QtensorQAOAExpectationValuesMAXCUT(problem, p, gamma=gamma, beta=beta, pbar=False)
...
def individual_MAXCUT_QAOA_optimization_single_initialization(G_num, n, regularity, max_p, initialization, opt, learning_rate):
    ...
    if initialization == 'analytic':
        ...
        pickle.dump(dictionary_fixed_angles, open(f"nodes_{n}_reg_{regularity}_graph_{G_num}_initialization_{initialization}.pkl", 'wb'))
        ...
        pickle.dump(dictionary_fixed_angles_optimization, open(f"nodes_{n}_reg_{regularity}_graph_{G_num}_initialization_{initialization}_opt_{opt}_lr_{learning_rate}.pkl", 'wb'))
        return dictionary_fixed_angles_optimization   
