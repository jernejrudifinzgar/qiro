...
from Calculating_Expectation_Values import SingleLayerQAOAExpectationValues, QtensorQAOAExpectationValuesMIS,QtensorQAOAExpectationValuesMAXCUT, QtensorQAOAExpectationValuesQUBO, QtensorQAOAExpectationValuesQUBO_GPU
...
def execute_RQAOA_single_instance_recalculation_GPU(n, p, run, iteration, recalculation, version, output_results=False):
    my_path = os.path.dirname(__file__)
    my_path = os.path.dirname(my_path)
    reg = 3
    seed = 666
    ...
    problem = Generator.MAXCUT(G)
    expectation_values_qtensor = QtensorQAOAExpectationValuesQUBO_GPU(problem, p, initialization='fixed_angles_optimization', opt=torch.optim.SGD, opt_kwargs=dict(lr=0.0001))
    RQAOA_qtensor = RQAOA_recalculate(expectation_values_qtensor, 4, iteration, recalculations=recalculation, type_of_problem="MAXCUT")
    time_start = time()
    cuts_qtensor, solution_qtensor = RQAOA_qtensor.execute()
    time_end = time()
