            ...
            for variation in variations:
                ...
                if variation in ['QIRO', 'MMQ']:
                    ...
                    for run in runs:
                        for iteration in iterations:
                            try:
                                with open(my_path + f"/data/results_run_{run}_n_{n}_p_{p}_initialization_{initialization}_variation_{variation_2}_version_1.pkl", 'rb') as file:
                                    data = pickle.load(file)

                                if p==1:
                                    MIS_size_list_n.append(data['size_solution_single']/MIS_size_exact_2[run])
                                    list_graphs_qtensor.append(run)
                                else:
                                    MIS_size_list_n.append(data['size_solution_qtensor']/MIS_size_exact_2[run])
                                    list_graphs_qtensor.append(run)
...
def plot_MIS_size_per_n(ns, ps, runs, version, iterations, initialization, variations, regularity):
    ...
    for p in ps:
        ...
        for n in ns:
            ...
            for run in runs:
                for iteration in iterations:
                    with open(my_path + f"/data/version_{version}/results_run_{run}_iteration_{iteration}_n_{n}_p_{1}_initialization_{initialization}_variation_QIRO_version_{version}.pkl", 'rb') as file:
                        data = pickle.load(file)
                    MIS_size_greedy_list.append(data['size_solution_min_greedy']/MIS_size_exact[run])       
            average = np.average(MIS_size_greedy_list)
            std = np.std(MIS_size_greedy_list)
            ...
            for variation in variations:
                ...
                for run in runs:
                    for iteration in iterations:
                        try:
                            with open(my_path + f"/data/version_{version}/results_run_{run}_iteration_{iteration}_n_{n}_p_{p}_initialization_{initialization}_variation_{variation}_version_{version}.pkl", 'rb') as file:
                                data = pickle.load(file)
