...
class RQAOA_recalculate(QIRO):
    ...
    def __init__(self, expectation_values_input, nc, iteration, recalculations=6, type_of_problem="MAX_2_SAT", connectivity_output=False):
        """
        Initializes the RQAOA instance.
    ...
    def execute(self):
        ...
        #while len(expect_val_dict)>self.nc or start==True:
            ...
            if step%self.recalculations==0:
                ...
                self.num_nodes_list.append(self.problem.graph.number_of_nodes())
            ...
            while ntc1 != mem[ntc1][0]:
                sign *= mem[ntc1][1]
                ntc1 = mem[ntc1][0]
            while ntc2 != mem[ntc2][0]:
                sign *= mem[ntc2][1]
                ntc2 = mem[ntc2][0]
            ...
            ntec1_name = translater_copy[ntc1]
            ntec2_name = translater_copy[ntc2]
            exp_value_coeff_name = (min(ntec1_name, ntec2_name), max(ntec1_name, ntec2_name))
            exp_value_coeff = (self.expectation_values.problem.position_translater.index(exp_value_coeff_name[0]), self.expectation_values.problem.position_translater.index(exp_value_coeff_name[1]))

            # Store the fixed correlation for later use in solution reconstruction
            self.fixed_correlations.append([exp_value_coeff_name, int(sign), max_exp_value])

            # If we're fixing a single-point correlation
            if len(exp_value_coeff) == 1:
                # Update the Hamiltonian to reflect the fixed single-point correlation
                self.update_single([exp_value_coeff[0]], sign)
                # Update lists to reflect that a variable has been fixed
                self.problem.remain_var_list.remove(exp_value_coeff_name[0])
                self.problem.position_translater.remove(exp_value_coeff_name[0])
            # If we're fixing a two-point correlation
            else:
                # Update the Hamiltonian to reflect the fixed two-point correlation
                self.update_correlation(list(exp_value_coeff), sign)
                # Update lists to reflect that a variable has been fixed
                self.problem.remain_var_list.remove(exp_value_coeff_name[1])
                self.problem.position_translater.remove(exp_value_coeff_name[1])
            
            mem[max(ntc1, ntc2)] = [int(min(ntc1, ntc2)), int(sign)]
    ...
    def update_correlation(self, exp_value_coeff, exp_value_sign):
        ...
        for node in self.problem.graph.nodes():
            ...
            if connectivity>max_connectivity:
                max_connectivity=connectivity
        self.connectivity.append(list_connectivity)
...
class RQAOA_recalculate_v2(QIRO):
    """
    RQAOA is a subclass of QIRO, designed to implement the Recursive Quantum Approximate Optimization Algorithm for combinatorial optimization problems.
    ...
    def execute(self):
        ...
        for step in range(self.nq):
            ...
            exp_value_coeff = max(expect_val_dict, key=lambda y: abs(expect_val_dict[y]))
            ...
            # If we're fixing a two-point correlation
                ...
                correlations_copy = expect_val_dict.copy()
                del expect_val_dict[exp_value_coeff]

                correlations_new = {}
                for correlation in correlations_copy.keys():
                    if min(list(correlation))==max(list(exp_value_coeff)) or max(list(correlation))==max(list(exp_value_coeff)):
                        pass
                    elif min(list(correlation))>max(list(exp_value_coeff)):
                        correlations_new[frozenset({min(list(correlation))-1, max(list(correlation))-1})] = expect_val_dict[correlation]
                    elif max(list(correlation))>max(list(exp_value_coeff)):
                        correlations_new[frozenset({min(list(correlation)), max(list(correlation))-1})] = expect_val_dict[correlation]
                    else:
                        correlations_new[frozenset({min(list(correlation)), max(list(correlation))})] = expect_val_dict[correlation]
                
                expect_val_dict = correlations_new.copy()
